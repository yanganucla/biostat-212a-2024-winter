...
$$\text{RSS} = \sum_{i=1}^{n} (y_i - \hat{y_i})^2 = (\mathbf{y} - \hat{\mathbf{y}})^T (\mathbf{y} - \hat{\mathbf{y}})$$
$$\text{TSS} = \sum_{i=1}^{n} (y_i - \bar{y})^2 = (\mathbf{y} - \bar{\mathbf{y}})^T (\mathbf{y} - \bar{\mathbf{y}})$$

$$\operatorname{Cor}(\mathbf{y}, \hat{\mathbf{y}}) = \frac{\operatorname{Cov}(\mathbf{y}, \hat{\mathbf{y}})}{\sigma_{\mathbf{y}} \sigma_{\hat{\mathbf{y}}}}$$
$$\operatorname{Cov}(\mathbf{y}, \hat{\mathbf{y}}) = \frac{1}{n} (\mathbf{y} - \bar{\mathbf{y}})^T (\hat{\mathbf{y}} - \bar{\mathbf{y}})$$
$$\operatorname{Cor}(\mathbf{y}, \hat{\mathbf{y}}) = \frac{\frac{1}{n} (\mathbf{y} - \bar{\mathbf{y}})^T (\hat{\mathbf{y}} - \bar{\mathbf{y}})}{\sigma_{\mathbf{y}} \sigma_{\hat{\mathbf{y}}}}$$
$$\operatorname{Cor}(\mathbf{y}, \hat{\mathbf{y}}) = \frac{\frac{1}{n} (\mathbf{y} - \bar{\mathbf{y}})^T (\mathbf{y} - \bar{\mathbf{y}})}{\sigma_{\mathbf{y}} \sigma_{\hat{\mathbf{y}}}} \cdot \frac{\frac{1}{n} (\hat{\mathbf{y}} - \bar{\mathbf{y}})^T (\hat{\mathbf{y}} - \bar{\mathbf{y}})}{\frac{1}{n} (\hat{\mathbf{y}} - \bar{\mathbf{y}})^T (\hat{\mathbf{y}} - \bar{\mathbf{y}})}$$
$$\operatorname{Cor}(\mathbf{y}, \hat{\mathbf{y}}) = \frac{\text{TSS}}{\sigma_{\mathbf{y}} \sigma_{\hat{\mathbf{y}}}} \cdot \frac{\text{RSS}}{\text{RSS}} = \frac{\text{TSS}}{\text{RSS}}$$
$$R^2 = 1 - \frac{\text{RSS}}{\text{TSS}} = 1 - \frac{\text{TSS}}{\text{RSS}} = [\operatorname{Cor}(\mathbf{y}, \hat{\mathbf{y}})]^2$$
...
r.squared <- summary(multi_fit)$r.squared
#Calculating the correlation between the response vector and the fitted values
correlation <- cor(Boston$crim, multi_fit$fitted.values)
#Calculating the square of the correlation
correlation.squared <- correlation^2
...
From the above code, we can see that the two values are equal. This means that $R^2$ is equal to the correlation between the response vector and the fitted values.
